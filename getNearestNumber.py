...
def getNearestNumInternal(list, k):
    length = len(list)
    if length == 1:
        if list[0] <= k:
        ...
        mid = length // 2
        if list[mid] <= k:
            return getNearestNumInternal(list[mid:], k)
        else:
            return getNearestNumInternal(list[:mid+1], k)


def getNearestNum(list, k):
    '''
    n개의 정렬된 숫자가 list로 주어지고, 숫자 k이 
    주어질 때, n개의 숫자 중에서 k과 가장 가까운 숫자를 
    반환하는 함수를 작성하세요.
    '''
    numbers = getNearestNumInternal(list, k)
    min = numbers[0]
    max = numbers[1]
